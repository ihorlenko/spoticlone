...
| 2026-06-10 | Фаза 15 (Visual Polish): Helpers/DurationConverter.cs (int ms → "m:ss"); HomePage.xaml — 6 змін: (1) заголовки секцій FontSize=21 + HorizontalStackLayout з SpotifyGreen BoxView accent 4px; (2) gradient overlay на картках треків і альбомів (Grid+Image fullheight + LinearGradientBrush Transparent→#EE282828 + текст поверх InputTransparent); (3) артисти — коло 115px + FontAttributes=Bold + Shadow glow SpotifyGreen Opacity=0.25; (4) DurationMs форматований через DurationConverter на картці треку; (5) Genre badge — Border RoundRectangle 10 #CC121212 top-left на картці треку; (6) VerticalStackLayout Spacing=24, Padding bottom=40, всі TapGestureRecognizer/команди збережені. Build 0 errors 0 warnings | — |
| 2026-06-10 | Фаза 15 (List alignment): SearchPage.xaml — усі 3 секції результатів (треки/артисти/альбоми) приведено до еталону HistoryPage: обкладинка 52→56px, BackgroundCard→BackgroundElevated, Grid Padding="0,8"→Padding="4", обгортка в CardBorder+Margin="0,0,0,4", GestureRecognizer на TapGestureRecognizer переміщено на Border (треки). LibraryPage.xaml — Playlists: колонка іконки 44→56px, FontSize іконки 28→32. LikedTracks вже відповідав еталону. Build 0 errors 0 warnings | — |
| 2026-06-11 | Фаза 15 (iOS like icon fix): HomePage.xaml — кнопка лайку: Text="♡"→"&#xe87e;" + додано FontFamily="MaterialIcons" на кнопку; DataTrigger Value=True: Text="♥"→"&#xe87d;" + додано Setter FontFamily=MaterialIcons; додано парний DataTrigger Value=False з &#xe87e; + FontFamily=MaterialIcons. PlayerPage.xaml і LibraryPage.xaml вже коректні (MaterialIcons + &#xNNNN;). Build 0 errors 0 warnings | — |
| 2026-06-11 | Фаза 15 (Size polish): SearchPage.xaml вже відповідав еталону HistoryPage (56px, CardBorder, Padding=4, FontSize=15) — змін не потрібно. LibraryPage.xaml — +20% по всіх розмірах: LikedTracks cover 56→68px, cols "56,*,44,44"→"68,*,52,52", Padding 4→5, title 15→18, subtitle CaptionLabel→FontSize=14+TextMuted inline, Spacing 3→4, кнопки 44→52 FontSize 20→22; Playlists cols "56,*,44"→"68,*,52", icon FontSize 32→38, title/subtitle/button аналогічно. Build 0 errors 0 warnings | — |
